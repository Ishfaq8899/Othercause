...
#'
#'
#' @param data A data frame containing MCED cancer mortality data.
#' It should include a column named `cancer_site`, which may be created by mapping
#' values from the `ICD-10 113 Cause List` column to standardized cancer site names.

#'
#' @param cancer_sites A vector of selected cancer type names to filter. These names
#'                     should match the values in the `ICD-10 113 Cause List` column of
#'                     the data frame.
#'
...
#'
#' @param othercause_death_table A data frame with survival probabilities for causes other
#'                             than the selected cancers.
#'                               Includes columns: age, sex, year, other_cause_rate, surv.
